...
import AccountData from "@/services/instagram/AccountInfo";
...
export default async function Instagram({ username, social }: InstagramProps) {
	const data = await AccountData(username);
	if (!data.status) return null;

	const subs = millify(Number(data.data?.follower_count), { precision: 2, locales: "es-ES" });
		...
		<a href={social} target="_blank" rel="noopener">
			<div className="button-style button-transition flex h-14 w-full items-center justify-between rounded-full px-4">
				<div className="flex select-none items-center justify-center gap-2">
					...
					{/* Title */}
					<h2 className="button-title">{data.data.page_name}</h2>

					{/* Followers */}
					{data.data.follower_count && <span className="button-followers">{subs}</span>}
				</div>
